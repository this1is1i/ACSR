...
> 综合 `docs/draw/02-frontend-package.drawio`、`03-backend-package.drawio`、`04-python-package.drawio` 三张包图，以模块包粒度重绘调用链路。
...
graph TD
    ...
    VUE -->|"REST /api/* + WebSocket /ws-messages"| SPRING
    SPRING -->|"HTTP POST /recommend /train /learning-path ..."| FAST
    SPRING -->|"Bolt Cypher"| NEO4J
    FAST -->|"PyMySQL"| MYSQL
    FAST -->|"Bolt"| NEO4J
```
...
```mermaid
graph TD
    subgraph "Frontend 源码 src/"
        subgraph "入口"
            MAIN["main.js<br/>全局样式+ElementPlus+Pinia"]
            APP["App.vue<br/><router-view/>"]
        end

        subgraph "路由与状态"
            ROUTER["router/index.js<br/>12 routes + guards"]
            STORE["store/userStore.js<br/>Pinia: token + userInfo"]
        end

        subgraph "页面 views/"
            HOME["Home.vue"]
            SEARCH["Search.vue"]
            PAPER["PaperDetail.vue"]
            KG["KnowledgeGraph.vue"]
            COMM["Community.vue"]
            PROFILE["Profile.vue"]
            EDIT["EditProfile.vue"]
            LOGIN["Login.vue"]
            CHAT["RealtimeChat.vue"]
            ADMIN["AdminConsole.vue"]
        end

        subgraph "组件 components/"
            SIDEBAR["Sidebar.vue"]
            PAPERCARD["PaperCard.vue"]
            RECLIST["RecommendList.vue"]
            RECSTREAM["home/RecommendationStream.vue"]
            PATHPANEL["home/LearningPathPanel.vue"]
            FILTER["search/SearchFilterRail.vue"]
            RESULT["search/SearchResultCard.vue"]
            PATHRAIL["paper/PaperPathRail.vue"]
            INSIGHT["path/PathInsightRail.vue"]
            CONVRAIL["chat/ConversationRail.vue"]
            COMTREE["CommunityCommentTree.vue"]
            PAGEHDR["layout/PageHeader.vue"]
            HERO["admin/AdminCockpitHero.vue"]
            KPI["admin/AdminKpiGrid.vue"]
        end

        subgraph "数据层"
            API["api/{user,paper,recommend,community,message,knowledge,visualization,admin,claim}.js"]
            REQ["utils/request.js<br/>axios + JWT拦截器"]
            AUTH["utils/auth.js<br/>localStorage读写"]
            PAPERU["utils/paper.js<br/>字段清洗+搜索状态"]
            PATHU["utils/path.js<br/>学习路径格式化"]
        end

        subgraph "样式"
            TOKENS["styles/tokens.css<br/>65 CSS变量 + 双主题"]
            LAYOUT["styles/layout-system.css<br/>page-root + 响应式"]
            GLOBAL["style.css<br/>reset + .card/.glass/.btn"]
        end
    end
...
```mermaid
graph TD
    subgraph "Spring Boot Backend com.example.research/"
        subgraph "Web 层"
            CTRL["controller/<br/>11 个 REST 控制器"]
            WS["MessageWebSocketController<br/>STOMP 3 通道"]
        end

        subgraph "业务层"
            USERSVC["UserServiceImpl"]
            RECSVC["RecommendServiceImpl"]
            COMMSVC["CommunityServiceImpl"]
            ADMINSVC["AdminServiceImpl"]
            MSGSVC["PrivateMessageServiceImpl"]
            VISUALSVC["VisualizationServiceImpl"]
        end

        subgraph "数据层"
            MAPPER["repository/<br/>11 个 MyBatis-Plus Mapper"]
            ENTITY["entity/<br/>13 个 DB 实体"]
            DTO["dto/<br/>6 个 DTO 容器类"]
        end

        subgraph "跨服务"
            PYCLIENT["client/PythonRecClient<br/>5 HTTP 端点 + Jackson映射"]
            GRAPHSVC["graph/GraphPaperService<br/>Neo4j Cypher 直连"]
        end

        subgraph "基础设施"
            SECURITY["config/SecurityConfig<br/>JWT Filter + 白名单"]
            WEBSOCKET["config/WebSocketConfig<br/>STOMP Broker"]
            CORS["config/CorsConfig"]
            JWT["util/JwtUtil<br/>签发 + 验证 + 解析"]
            RESULT["util/Result<T><br/>统一响应信封"]
        end
    end
    ...
    SECURITY --> JWT
    WEBSOCKET --> WS
    GRAPHSVC -.->|Bolt| N4J[("Neo4j")]
    PYCLIENT -.->|HTTP| PY[("Python :8000")]
```
...
```mermaid
graph TD
    subgraph "Python RL Service"
        CFG["config.py<br/>@dataclass Config<br/>全局超参+连接"]

        subgraph "API 入口"
            API["api/server.py<br/>FastAPI 6 endpoints"]
            TRAIN["train.py<br/>AC 训练主循环"]
        end

        subgraph "推荐管线"
            SVC["services/recommendation_service.py<br/>编排: 特征→召回→排序→解释"]
            FEAT["features/feature_builder.py<br/>用户特征: MySQL→96维状态"]
            CAND["recommender/candidate_generator.py<br/>候选: hybrid/similarity/popular"]
            RANK["recommender/ranker.py<br/>RLRanker: 3项打分+门控+归一化"]
        end

        subgraph "模型"
            AGENT["agent.py<br/>ActorCriticAgent"]
            ACTOR["models/actor.py<br/>MLP: [state+paper]→logit"]
            CRITIC["models/critic.py<br/>MLP: state→V(s)"]
        end

        subgraph "RL 环境"
            ENV["env/rec_env.py<br/>Gym式 Reset/Step"]
            RWD["utils/reward.py<br/>6维加权奖励"]
        end

        subgraph "知识图谱"
            KGB["knowledge_graph/kg_builder.py<br/>内存图构建"]
            KGE["knowledge_graph/kg_embedder.py<br/>10→32投影"]
            KGQ["knowledge_graph/graph_query.py<br/>BFS+解释+聚类"]
            KGS["knowledge_graph/graph_storage.py<br/>JSON/Pickle 持久化"]
        end

        subgraph "学习路径"
            LPB["learning_path/path_builder.py<br/>4层节点生成"]
            LPP["learning_path/propagation.py<br/>掌握度传播+颜色"]
        end

        subgraph "数据访问"
            MYS["data/mysql_data.py<br/>PyMySQL"]
            MOCK["data/mock_data.py<br/>训练回退生成器"]
            LOAD["dataset/aminer_loader.py<br/>AMiner JSON 解析"]
        end
    end
...
| 层 | 核心依赖 | 数量 |
|----|---------|:---:|
| 前端 npm | vue, vue-router, pinia, element-plus, axios, 3d-force-graph, three, @stomp/stompjs, sockjs-client, vite | 12 |
| 后端 Maven | spring-boot-{web,security,validation,websocket}, mybatis-plus, mysql-connector, neo4j-java-driver, jjwt, httpclient5, lombok | 11 |
| Python pip | fastapi, uvicorn, pydantic, torch, numpy, pymysql, neo4j | 7 |
