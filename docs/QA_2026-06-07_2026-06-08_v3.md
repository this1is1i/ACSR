...
> 综合 `docs/draw/02-frontend-package.drawio`、`03-backend-package.drawio`、`04-python-package.drawio` 三张包图。仅展示本地代码包之间的依赖，不含外部库。

### 前端：`frontend/src/` 源码包依赖
...
graph TD
    ...
    subgraph "api/ 接口"
        API_USER["user.js"]
        API_PAPER["paper.js"]
        API_REC["recommend.js"]
        API_COMM["community.js"]
        API_MSG["message.js"]
        API_KG["knowledge.js"]
        API_VIZ["visualization.js"]
        API_ADMIN["admin.js"]
        API_CLAIM["claim.js"]
    end
    ...
    subgraph "router/ + store/"
        ROUTER["router/index.js"]
        STORE["store/userStore.js"]
    end

    subgraph "styles/ 样式"
        STYLE["style.css"]
        TOKENS["styles/tokens.css"]
        LAYOUT["styles/layout-system.css"]
    end
...
### 后端：`com.example.research/` Java 包依赖

```mermaid
graph TD
    subgraph "controller/ 控制器"
        CTRL_USER["UserController"]
        CTRL_PAPER["PaperController"]
        CTRL_REC["RecommendController"]
        CTRL_COMM["CommunityController"]
        CTRL_MSG["PrivateMessageController"]
        CTRL_WS["MessageWebSocketController"]
        CTRL_BEH["BehaviorController"]
        CTRL_KG["KnowledgeController"]
        CTRL_VIZ["VisualizationController"]
        CTRL_CLAIM["ClaimController"]
        CTRL_ADMIN["AdminController"]
    end

    subgraph "service/impl/ 业务逻辑"
        SVC_USER["UserServiceImpl"]
        SVC_REC["RecommendServiceImpl"]
        SVC_COMM["CommunityServiceImpl"]
        SVC_ADMIN["AdminServiceImpl"]
        SVC_MSG["PrivateMessageServiceImpl"]
        SVC_VIZ["VisualizationServiceImpl"]
    end

    subgraph "repository/ 数据访问"
        MAPPER_USER["UserMapper"]
        MAPPER_PAPER["PaperMapper"]
        MAPPER_BEH["BehaviorLogMapper"]
        MAPPER_MSG["PrivateMessageMapper"]
        MAPPER_CONTACT["UserContactMapper"]
        MAPPER_POST["PostMapper"]
        MAPPER_COMMENT["CommentMapper"]
        MAPPER_LIKE["PostLikeMapper"]
        MAPPER_CLAIM["PaperAuthorClaimMapper"]
        MAPPER_FAV["FavouriteMapper"]
        MAPPER_INTEREST["UserInterestHistoryMapper"]
    end

    subgraph "entity/ + dto/"
        ENTITY["entity/{User,Paper,Post,...} 13实体"]
        DTO["dto/{UserDto,CommunityDto,...} 6容器"]
    end

    subgraph "跨服务 client/ + graph/"
        PYCLIENT["client/PythonRecClient"]
        GRAPHSVC["graph/GraphPaperService"]
    end

    subgraph "config/ + util/"
        CONFIG["config/{SecurityConfig,WebSocketConfig,CorsConfig}"]
        UTIL["util/{JwtUtil,Result}"]
    end
    ...
    SVC_USER --> MAPPER_FAV
    SVC_USER --> MAPPER_INTEREST
    SVC_COMM --> MAPPER_POST
    SVC_COMM --> MAPPER_COMMENT
    SVC_COMM --> MAPPER_LIKE
    SVC_MSG --> MAPPER_MSG
    SVC_MSG --> MAPPER_CONTACT
    SVC_MSG --> MAPPER_USER
    SVC_ADMIN --> MAPPER_POST
    SVC_ADMIN --> MAPPER_PAPER
    SVC_ADMIN --> MAPPER_CLAIM
    SVC_ADMIN --> MAPPER_USER
...
graph TD
    ...
    subgraph "services/ 编排"
        SVC["services/recommendation_service.py"]
    end

    subgraph "推荐管线"
        FEAT["features/feature_builder.py"]
        CAND["recommender/candidate_generator.py"]
        RANK["recommender/ranker.py"]
    end
    ...
    subgraph "知识图谱"
        KGB["knowledge_graph/kg_builder.py"]
        KGE["knowledge_graph/kg_embedder.py"]
        KGQ["knowledge_graph/graph_query.py"]
        KGS["knowledge_graph/graph_storage.py"]
    end

    subgraph "学习路径"
        LPB["learning_path/path_builder.py"]
        LPP["learning_path/propagation.py"]
    end

    subgraph "数据访问"
        MYS["data/mysql_data.py"]
        MOCK["data/mock_data.py"]
        LOAD["dataset/aminer_loader.py"]
    end
